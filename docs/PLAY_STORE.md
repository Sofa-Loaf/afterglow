...
| Version | `0.1.3` (`app.json` / `package.json`), Android `versionCode` `4` |
| Architecture | **New Architecture on.** No Reanimated / worklets. See note below. |
| Privacy policy URL | **https://28to3.me/apps/afterglow.html#privacy** |
...
**Play architecture (why Reanimated is gone)**

v0.1.1 (`newArchEnabled: true`, Reanimated 4 + worklets) and v0.1.2 (`newArchEnabled: false`, Reanimated 3.19.1) both still instant-closed on John’s Pixel 10 Pro (“keeps stopping”). Afterglow does not animate. `expo-router` lists Reanimated as an **optional** peer; this AAB omits it so `libreanimated.so` / `libworklets.so` never load before JS. New Architecture stays **on** (Expo SDK 54 default). Gesture-handler stays for native-stack.
